...
app.get('/Generator', checkNotAuthenticated, function(req, res) {
	postgres
        .query(`
            SELECT prompt_id, initialText, Prompt
            FROM savedPrompts
            WHERE user_id = ${req.user.id}
        ;`)
        .then((results, err) => {
...
app.post('/savePrompts', checkNotAuthenticated, function(req,res) {
	postgres.query(`
		INSERT INTO savedPrompts(user_id, prompt_id, initialText, Prompt)
		VALUES (${req.user.id}, ${req.body.Prompt_id}, '${req.body.initText}', '${req.body.prompt}')
	;`)
	.catch(error => console.log(error));
});

app.post('/loadPrompts', checkNotAuthenticated, function(req,res) {
	postgres.query(`
		SELECT initialText, Prompt
		FROM savedPrompts
		WHERE user_id = ${req.user.id}
		AND prompt_id = ${req.body.num}
	;`)
	.then((results,err) => {
		res.send({initialTxt: results.rows[0].initialtext, Prompt: results.rows[0].prompt});
	})
	.catch(err => {
		console.log(err)
	});
...
app.post('/deletePrompts', checkNotAuthenticated, function(req, res) {
	postgres.query(`
		DELETE FROM savedPrompts
		WHERE user_id = ${req.user.id}
		AND prompt_id = ${req.body.num}
	;`)
	.then((results,err) => {
