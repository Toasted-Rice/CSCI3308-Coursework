function createButton(context, num) {
    ...
    button.onclick = loadPrompt(num);

    var dbutton = document.createElement("button");
    dbutton.setAttribute("style","float: right;");
    dbutton.onclick = deletePrompt(num);
    dbutton.innerHTML = "<img src='/resources/img/deletePromptImg.png' height='12px' width='12p'>"
...
function savePrompt(){
    ...
    document.getElementById('promptDisplay').innerText == 'Select generated prompt to be shown here' ||
    document.getElementById('promptDisplay').innerText == 'Loading...') {

    }
    else {
        let saveLi = document.getElementById('savedPromptList');
        if(saveLi.childNodes.length == 0){
            num = (saveLi.childNodes.length);
        }
        else{
            num = (saveLi.childNodes.length - 1);
        }
        
        createButton(saveLi,num);
...
function loadPrompt(num){
    $.ajax({
        ...
        data: {num: num},
        success: (results) => {
                document.getElementById('txtBox').value = results.initialTxt;
                document.getElementById('promptDisplay').innerText = results.Prompt;
        }
    });
    return false;
}

function deletePrompt(num){
    $.ajax({
        url: '/deletePrompts',
        type: 'POST',
        cache: false,
